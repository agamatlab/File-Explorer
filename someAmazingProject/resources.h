...
void emptyItem(string path) {
	for (const auto& file : directory_iterator(path)) {
		...
		if (is_directory(currentPath)) emptyItem(currentPath);
		remove(currentPath);
	}
}

bool startsWith(string main, string prefix) {
	string sourcePath = main + (main.back() == '\\' ? "" : "\\") +
		s_fileName.substr(s_fileName.rfind('\\') + 1);
...
void cloneFolder(string from, string to) {
	...
	for (const auto& file : directory_iterator(from))
		...
		else filesystem::copy(file.path().string(), to);
...
string miniEnvironment(string startingPath) {
	...
	// Esas Program kimi Directory'ler arasi kecid eden function'dur
	// Sadece mueyyen meqsed ucun sadeleshdirilmis versiyasidir.

	// Move eden zaman elave davranislar ucun global'da bool saxlaya bilerdim
	// IsMove meselen ve move'a uygun davranishlar ucun her defe yoxlayardim 
	// if(IsMove) {} lakin esas functionlarda her defe bir komanda edende bu
	// serti yoxlamasin deye miniEnvironment (kicik muhit) function'u  yazdim.
	...
	while (true)
	{
		...
		if (kb == 224 || kb == -32) {
			...
			case ENTER:
				...
				if (is_directory(currentFileName)) { startingPath = currentFileName; choice = 0; }
				break;
			case 'm':
			case 'M':
