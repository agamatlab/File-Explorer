...
int main(){
	...
	while (true)
	{
			...
			try 
				...
				{ cout << "Specifed Directory does not EXIST : " << dir << endl; reDirectory(); continue;}
		...
		try { manageConsoleInput(choice, i); }
		catch (const std::exception& ex) { cout << ex.what() << endl; cin.get(); }
